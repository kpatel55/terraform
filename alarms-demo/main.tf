...
resource "aws_sns_topic" "sns_notify_alarm" {
...
resource "aws_cloudwatch_composite_alarm" "ec2_alarm" {
  ...
  alarm_actions = [aws_sns_topic.sns_notify_alarm.arn]
  ok_actions = [aws_sns_topic.sns_notify_alarm.arn]
  ...
  depends_on = [
    module.alarm_module,
    aws_sns_topic.sns_notify_alarm
  ]
}

output "sns_out" {
  value = [module.alarm_module.alarms_out , aws_sns_topic.sns_notify_alarm.arn]
}
